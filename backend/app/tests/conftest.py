...
async def db_client(  # type: ignore[no-untyped-def]
    ...
    portal-тред), а async db-фикстуры pytest-asyncio живут в loop'е теста —
    asyncpg-коннект из db_session_fixture ловит «Task ... got Future attached
    to a different loop». ASGITransport выполняет запрос в том же loop'е, что
    и фикстуры (ровно так работают гейты backend/scripts/verify_admin_*.py).
    Lifespan при этом не гоняется — для этих тестов он и не нужен (только
