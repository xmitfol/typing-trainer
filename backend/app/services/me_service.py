...
async def patch_profile(session: AsyncSession, user: User, patch: dict) -> User:
...
async def patch_settings(session: AsyncSession, user_id: UUID, patch: dict) -> UserSettings:
...
async def get_achievements(session: AsyncSession, user_id: UUID) -> list:
...
def _normalize_progress_bundle(progress: dict, default_tier: str) -> list[dict]:
...
async def migrate_guest(
    ...
    for h in guest_data.history or []:
        if not isinstance(h, dict):
            continue
        lesson_num = h.get("lesson") or h.get("lesson_num")
        try:
