...
async def main() -> int:
    ...
    async with httpx.AsyncClient(transport=transport, base_url=base) as client:
        ...
        check(
            r.status_code == 200 and body.get("otpauth_uri", "").startswith("otpauth://")
            and len(secret) >= 16,
        ...
        async with factory() as s:
            row = (await s.execute(
                select(Admin2FA).where(Admin2FA.user_id == super_id)
            )).scalar_one_or_none()
        check(row is not None and row.enabled is False,
        ...
        check(r.status_code == 403 and code == "TOTP_REQUIRED",
        ...
        code = (r.json().get("detail", {}) or {}).get("code")
        check(r.status_code == 403 and code == "REAUTH_FAILED",
              "reauth неверный пароль → 403 REAUTH_FAILED", f"code={code}")
        await clear_fail_rl(redis, super_id)
        ...
        check(r.status_code == 200 and "reauth_token" in r.json(),
              "reauth с recovery-кодом → 200 токен", f"{r.status_code}")
        await clear_fail_rl(redis, super_id)
        r = await client.post("/api/v1/admin/reauth", cookies=cookies_for(super_id),
                              json={"password": PW, "totp_code": rc0})
        code = (r.json().get("detail", {}) or {}).get("code")
        check(r.status_code == 403 and code == "TOTP_INVALID",
              "повтор того же recovery → 403 (one-time consumed)", f"code={code}")
        ...
        check(r.status_code == 403 and code == "ADMIN_FORBIDDEN",
              "enroll support (не superadmin) → 403 ADMIN_FORBIDDEN", f"{r.status_code} code={code}")
        ...
        r = await client.post("/api/v1/admin/reauth", cookies=cookies_for(support_id),
                              json={"password": PW})  # без totp_code
        check(r.status_code == 200 and "reauth_token" in r.json(),
        ...
        last = None
        for _ in range(6):
            last = await client.post("/api/v1/admin/reauth", cookies=cookies_for(super_id),
                                     json={"password": PW, "totp_code": "111111"})
        code = (last.json().get("detail", {}) or {}).get("code")
        check(last.status_code == 429 and code == "RATE_LIMITED",
        ...
        try:
            ...
            r = await client.post("/api/v1/admin/reauth", cookies=cookies_for(super2_id),
                                  json={"password": PW})
            code = (r.json().get("detail", {}) or {}).get("code")
            check(r.status_code == 403 and code == "TOTP_ENROLLMENT_REQUIRED",
        ...
        r = await client.post("/api/v1/admin/2fa/disable", cookies=cookies_for(super_id),
                              headers={"X-Admin-Reauth": reauth_tok})
        check(r.status_code == 200 and r.json().get("enabled") is False,
              "disable (superadmin + reauth) → 200 enabled=False", f"{r.status_code}")
        async with factory() as s:
            row = (await s.execute(select(Admin2FA).where(
                Admin2FA.user_id == super_id))).scalar_one_or_none()
        ...
        r = await client.post("/api/v1/admin/reauth", cookies=cookies_for(super_id),
                              json={"password": PW})
        check(r.status_code == 200 and "reauth_token" in r.json(),
              "после disable → reauth по паролю проходит (без TOTP)", f"{r.status_code}")
