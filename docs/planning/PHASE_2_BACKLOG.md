...
### Адаптивные повторы / weak-keys  📐 спроектировано, отложено как доп-функционал
**Размер:** Phase 1 (rule-based, клиентский) ~1 нед; Phase 2 ML / Phase 3 LLM — позже
**Статус:** дизайн готов — [docs/spec/methodology/adaptive_repetition_spec.md](../spec/methodology/adaptive_repetition_spec.md) (Ася).
**Суть:** число повторов в гайдед-шагах подстраивается под per-key error-rate — где освоено, меньше; где слабо, больше / точечный дрилл (по методике СОЛО).
**Почему отложено (решение PO 2026-06-25):** сперва валидируем саму методику гайдед-уроков (лестница + постановка рук) на живых пользователях с фиксированными повторами; адаптивному движку для калибровки порогов всё равно нужны реальные данные использования, которых пока нет. AI — доп-фича (возможно Premium), не блокирует прототип.
**Когда строить:** после накопления usage-данных. Phase 1 checklist — в конце спека. Сейчас повторы в `data/lessons/tier1/lesson_01.json` зашиты статикой (placement 10 заходов, буквы ~20) — это сознательная заглушка под будущую адаптацию (`unit`+`baseReps` по спеку).
