...
1. **[★★★, первым] CI real-Docker** — `.github/workflows/` пусто (инфра-долг Sprint 1). Actions: поднять стек `docker compose --profile app up` + прогон gate-скриптов (`scripts/verify_*.py`, 30+) + `scripts/lint_lessons.py` + compile-checks. Владелец Борис. Множитель качества.
...
3. **Мелкий долг**: block_1 `text_sequence` (2 WARN линтера) регенерация; i18n-пробелы; дубли; `subscription_charges` admin-debug эндпоинт. Борис+Полина.
4. **E2E синка на всех тирах** (ru_teen/ru_kids/en_*) — после CI, в тот же pipeline. Борис.
...
> **В полёте на момент сворачивания сессии (2026-07-02/03):** запущены фоном Борис (#1 CI) и Алекс (#3 адаптив-движок). Их диффы могут лежать в рабочем дереве незакоммиченными — новая сессия: проверить/забрать их результат ИЛИ перезапустить по плану выше.
