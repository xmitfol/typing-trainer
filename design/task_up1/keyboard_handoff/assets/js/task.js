...
(function () {
  ...
  // ── Render target text ─────────────────────────────────────────────────
  // Char → finger map (ЙЦУКЕН) for tinting current char in target text
  const CHAR_FINGER = {
    'й':'pink','ф':'pink','я':'pink','ё':'pink','1':'pink','2':'orange','ц':'orange','ы':'orange','ч':'orange',
    'у':'green','в':'green','с':'green','3':'green','к':'blue','а':'blue','м':'blue','е':'blue','п':'blue','и':'blue','4':'blue','5':'blue',
    'н':'indigo','р':'indigo','т':'indigo','г':'indigo','о':'indigo','ь':'indigo','6':'indigo','7':'indigo',
    'ш':'green','л':'green','б':'green','8':'green','щ':'orange','д':'orange','ю':'orange','9':'orange',
    'з':'pink','ж':'pink','х':'pink','э':'pink','ъ':'pink','0':'pink','.':'pink',' ':'purple',
  };
  ...
  function renderTarget() {
    ...
    targetEl.innerHTML = t.split('').map((ch, i) => {
      ...
      const fin = (i === typed) ? CHAR_FINGER[ch.toLowerCase()] : null;
      const fattr = fin ? ` data-finger="${fin}"` : '';
      return `<span class="${cls}"${fattr}>${ch === ' ' ? '&nbsp;' : ch}</span>`;
    }).join('');
    countEl.textContent = `${typed}/${t.length}`;
    fillEl.style.width = `${(typed / t.length) * 100}%`;
    // Update keyboard highlight (next char to press) — only if finger-hint is ON
    const nextCh = t[typed];
    if (window.taskFingerHint && !window.taskFingerHint()) {
  ...
  // ── Toolbar: 4 functional controls ───────────────────────────────────────
  // 1. Finger-placement highlight — ON by default; OFF → no next-key hint.
  let fingerHint = true;
  document.getElementById('finger-hint-btn').addEventListener('click', (e) => {
    const btn = e.currentTarget;
    fingerHint = !fingerHint;
    btn.dataset.off = (!fingerHint).toString();
    btn.title = fingerHint ? 'Подсветка расстановки пальцев (вкл.)' : 'Подсветка расстановки пальцев (выкл.)';
    if (fingerHint) refreshHighlight();
    else { kb.removeAttribute('highlight-char'); }
  });

  // 2. Key sound — OFF by default; OFF → red badge on button.
  let soundOn = false;
  document.getElementById('sound-btn').addEventListener('click', (e) => {
    const btn = e.currentTarget;
    soundOn = !soundOn;
    btn.dataset.off = (!soundOn).toString();
    btn.title = soundOn ? 'Звук нажатия (вкл.)' : 'Звук нажатия (выкл.)';
  });
  window.taskSound = () => soundOn; // task input reads this to decide playback

  // 3. Metronome — OFF by default.
  let metroOn = false;
  document.getElementById('metro-btn').addEventListener('click', (e) => {
    const btn = e.currentTarget;
    metroOn = !metroOn;
    btn.dataset.off = (!metroOn).toString();
    btn.dataset.active = metroOn.toString();
    btn.title = metroOn ? 'Метроном (вкл.)' : 'Метроном (выкл.)';
  });

  // 4. Typing-window zoom — 70..150%, default 100%. Scales target text for low-vision.
  ...
  zoomMinus.addEventListener('click', () => { zoomLevel = Math.max(ZOOM_MIN, zoomLevel - ZOOM_STEP); applyZoom(); });
  zoomPlus.addEventListener('click', () => { zoomLevel = Math.min(ZOOM_MAX, zoomLevel + ZOOM_STEP); applyZoom(); });
  applyZoom();

  // Expose so loadExercise/render can respect the finger-hint toggle
  function refreshHighlight() {
    if (!fingerHint) { kb.removeAttribute('highlight-char'); return; }
    const t = currentExercise().text;
    const nextCh = t[typed];
    if (nextCh === ' ') kb.setAttribute('highlight-char', ' ');
    else if (nextCh) kb.setAttribute('highlight-char', nextCh);
    else kb.removeAttribute('highlight-char');
  }
  window.taskFingerHint = () => fingerHint;
