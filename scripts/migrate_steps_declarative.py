...
Мигрируем ТОЛЬКО простой периодический случай: target = N>=2 идентичных
групп через одиночный пробел («а а а», «кофе кофе кофе») → unit=группа,
baseReps=N, keys = уникальные буквы unit в нижнем регистре без пробела
(тот же алгоритм, что fallback stepKeys() движка; spec предпочитает ЯВНЫЙ
keys). Непериодические target («вол вал лов лава», чередования «а о а о»,
прогрессии) НЕ мигрируются: движок их не адаптирует и берёт legacy target
как есть — так и задумано (§5.1 backward-compat).
...
def decompose(target: str) -> tuple[str, int] | str:
    ...
    first = groups[0]
    if any(g != first for g in groups):
        preview = " ".join(groups[:4]) + ("…" if len(groups) > 4 else "")
        return f"группы различаются (не unit×N): «{preview}»"
    return first, len(groups)
